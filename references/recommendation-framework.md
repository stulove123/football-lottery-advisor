...
✅ 总结
[先用一句话收束本场核心判断。然后必须以多行字段卡片输出，不要写成一个长段落，不要用分号把字段串在同一行，不要把整张卡片整体加粗。

最终推荐卡：
- 比赛：[主队] vs [客队]
- 主推：[竞彩玩法 + 方向]
- 次选：[竞彩玩法 + 方向；没有则写“无，等待临场赔率/首发复核”]
- 比分参考：[1-3 个小范围比分]
- 总进球数参考：[0/1/2/3/4/5/6/7+]
- 定位：[主推/次选/谨慎/暂不推荐 + 信心档]
- 信息时间：[YYYY-MM-DD HH:mm 时区]
- 临场复核点：[首发、赔率、伤停、天气/场地等]
- 反向风险：[最可能打穿推荐的比赛路径]
- 风险提示：这是赛前概率判断，不是收益承诺。]
```
...
✅ 总结
[至少100个中文字符。先用一句话收束本场核心判断，然后必须以多行字段卡片输出，不要写成一个长段落，不要用分号把字段串在同一行，不要把整张卡片整体加粗。
...
- 信息时间：[YYYY-MM-DD HH:mm 时区]
- 临场复核点：[首发、赔率、伤停、天气/场地等]
- 反向风险：[最可能打穿推荐的比赛路径，例如热门早早进球后节奏失控、弱队红牌、主队边路被打穿、临场核心缺席、赔率大幅下调等]
- 风险提示：这是赛前概率判断，不是收益承诺；临场首发和赔率变化要重看。]
```
